...
int simpleGPU() {
    ...
    result =
        cufftXtMalloc(plan_input, (cudaLibXtDesc**)&d_out_filter_kernel, CUFFT_XT_FORMAT_INPLACE);
    if (result != CUFFT_SUCCESS) {
        printf("*XtMalloc failed\n");
        exit(EXIT_FAILURE);
    }
    ...
    result = cufftXtMemcpy(
        plan_input, d_filter_kernel, h_padded_filter_kernel, CUFFT_COPY_HOST_TO_DEVICE);
    if (result != CUFFT_SUCCESS) {
        printf("*XtMemcpy failed\n");
        exit(EXIT_FAILURE);
    }
    ...
    result = cufftXtMemcpy(
        plan_input, d_out_filter_kernel, d_filter_kernel, CUFFT_COPY_DEVICE_TO_DEVICE);
    if (result != CUFFT_SUCCESS) {
        printf("*XtMemcpy failed\n");
        exit(EXIT_FAILURE);
    }
    ...
    bool bTestResult = sdkCompareL2fe(
        (float*)h_convolved_signal_ref, (float*)h_convolved_signal, 2 * SIGNAL_SIZE, 1e-5f);
...
ComplexPointwiseMulAndScale(cufftComplex* a, cufftComplex* b, int size, float scale) {
